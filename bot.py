...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    keyboard = [
        ...
        [InlineKeyboardButton("📢 ÜCRETSİZ BİLGİLENDİRME", callback_data="ucretsiz_bilgi")],
    ...
    welcome_text = (
        ...
        "⚠️ <i>Tüm üyeliklerimiz 30 gün geçerlidir.</i>\n\n"
        "👇 <b>Bilgi almak ve denemek için ücretsiz bölümü kullanabilirsiniz:</b>"
    )
...
async def button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "ucretsiz_bilgi":
        info_text = (
            ...
            "• <b>Günlük Güncelleme:</b> Her gün yeni içerik eklenir.\n"
            "• <b>Sorunsuz Erişim:</b> Ödeme sonrası otomatik onay.\n"
            "• <b>Güvenli Ödeme:</b> Shopier güvencesiyle.\n\n"
            "⚠️ <b>DİKKAT:</b> Sistemi test etmeniz için size özel <b>5 adet ücretsiz mesaj/soru hakkı</b> tanımlanmıştır. "
            "Lütfen aklınızdaki soruları bu hak dahilinde sorunuz. Hakkınız dolduğunda sistem sizi kısıtlayacaktır."
        )
...
async def mesaj_kontrol(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Ücretli üyeleri kontrol et
    aktif_uyeler = load_data()
    if user_id in aktif_uyeler:
        return

    # Ücretsiz mesaj sayacı
    ...
    if current_count < 5:
        ...
        await update.message.reply_text(f"📥 Mesajınız alındı. Kalan Ücretsiz Hakkınız: {5 - current_count}")
        # Soruyu admin'e ilet
        await context.bot.send_message(chat_id=ADMIN_ID, text=f"📩 <b>Soru (@{update.effective_user.username}):</b>\n{update.message.text}", parse_mode="HTML")
    else:
        await update.message.reply_text(
            "❌ <b>Ücretsiz mesaj hakkınız dolmuştur!</b>\n\n"
            "Daha fazla içerik ve günlük güncellemeler için lütfen size uygun bir paket seçiniz.\n\n"
            "Paketleri görmek için /start yazın.", 
            parse_mode="HTML"
        )
